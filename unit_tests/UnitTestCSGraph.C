...
TEST(CSGraph, compute_row_pointers)
{
  unsigned N = 5;
  size_t nnz = 0;
  ...
  sierra::nalu::CSGraph::compute_row_pointers(rowPointers, rowLengths);
  EXPECT_EQ(nnz, rowPointers[N]);
}

Teuchos::RCP<sierra::nalu::CSGraph> create_graph(const std::vector<size_t>& rowLens)
{
  ...
  return Teuchos::rcp(new sierra::nalu::CSGraph(rowLengths));
}

TEST(CSGraph, construct)
{
  Teuchos::RCP<sierra::nalu::CSGraph> csg = create_graph({2, 3, 4});
...
TEST(CSGraph, insertIndicesNumDof1)
{
  Teuchos::RCP<sierra::nalu::CSGraph> csg = create_graph({2, 3, 4});
...
TEST(CSGraph, insertIndicesNumDof3)
{
  Teuchos::RCP<sierra::nalu::CSGraph> csg = create_graph({6, 9, 12});
